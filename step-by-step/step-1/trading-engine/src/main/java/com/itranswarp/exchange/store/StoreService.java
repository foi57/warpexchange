...
import com.itranswarp.exchange.message.event.EventEntity;
...
public class StoreService extends LoggerSupport {
    ...
    public List<AbstractEvent> loadEventsFrom(long lastEvenId){
        List<EventEntity> events = this.dbTemplate.from(EventEntity.class).where("sequenceId > ?",lastEvenId)
    ...
    public void insertIgnore(List<? extends EntitySupport> list) {dbTemplate.insertIgnore(list);}
}

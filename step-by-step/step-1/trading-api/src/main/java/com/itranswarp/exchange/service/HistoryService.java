...
import com.itranswarp.exchange.bean.SimpleMatchDetaiRecord;
...
public class HistoryService extends AbstractDbService {
    ...
    public List<SimpleMatchDetaiRecord> getHistoryMatchDetails(Long orderId){
        List<MatchDetailEntity> details = db.select("price","quantity","type").from(MatchDetailEntity.class).where("orderId = ?",orderId).orderBy("id").list();
        return details.stream().map(e -> new SimpleMatchDetaiRecord(e.price,e.quantity,e.type)).collect(Collectors.toList());
    }
}

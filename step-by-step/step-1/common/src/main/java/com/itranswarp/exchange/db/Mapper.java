...
public class Mapper <T>{
    ...
    public Mapper(Class<T> clazz) throws Exception{
        ...
        this.insertableProperties = all.stream().filter(AccessibleProperty::isIdentityId).collect(Collectors.toList());
        ...
        this.insertSQL = "INSERT INTO " + this.tableName + " (" + String.join(", ",this.insertableProperties.stream().map(p -> p.propertyName).toArray(String[]::new))
        ...
        this.updateSQL = "UPDATE " + this.tableName + " SET " + String.join(", ",this.updatableProperties.stream().map(p -> p.propertyName + " = ?").toArray(String[]::new)) + " WHERE " + this.id.propertyName + " = ?";

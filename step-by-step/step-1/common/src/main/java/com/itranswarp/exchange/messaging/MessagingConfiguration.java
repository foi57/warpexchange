...
public class MessagingConfiguration {
    ...
    @Bean
    KafkaTemplate<String,String> kafkaTemplate(@Autowired ProducerFactory<String,String> producerFactory){
        logger.info("init kafka template...");
        return new KafkaTemplate<>(producerFactory);
    }
    @Bean
    public ConsumerFactory<String,String> consumerFactory(@Value("${spring.kafka.bootstrap-servers}") String bootstrapServers,@Value("${spring.kafka.consumer.max-poll-records}") int batchSize) {
        logger.info("init kafka consumer from bootstrap servers: {}, batch-size: {}", bootstrapServers,
                Integer.valueOf(batchSize));
        return new DefaultKafkaConsumerFactory<>(consumerConfigs(bootstrapServers,batchSize));} //ConsumerFactory
